...
int CStudioHdr::GetNumSeq_Internal() const
{
	return GetNumSeq();
}
...
{
	return const_cast<mstudioseqdesc_t&>(pSeqdesc(i));
}

mstudioseqdesc_t& studiohdr_t::pSeqdesc_Internal(int i)
{
	return const_cast<mstudioseqdesc_t&>(pSeqdesc(i));
}

int V_UCS2ToUTF8(const wchar_t* src, char* dest, int maxlen) { return 0; }
int V_UnicodeToUTF8(const wchar_t* src, char* dest, int maxlen) { return 0; }
int V_UTF8ToUnicode(const char* src, wchar_t* dest, int maxlen) { return 0; }
...
bool CMemoryStack::Init(unsigned int a, unsigned int b, unsigned int c, unsigned int d) { return true; }
...
void KeyValues::RecursiveSaveToFile(IBaseFileSystem* filesystem, FileHandle_t f, CUtlBuffer* pBuf, int indentLevel, bool bWriteEmptySubkeys /*= false*/)
{
	// write header
	...
	for (KeyValues* dat = m_pSub; dat != NULL; dat = dat->m_pPeer)
		SaveKeyToFile(dat, filesystem, f, pBuf, indentLevel, false, false);

	// write tail
	WriteIndents(filesystem, f, pBuf, indentLevel);
	INTERNALWRITE("}\n", 2);
}
bool KeyValues::SaveToFile(IBaseFileSystem* filesystem, const char* resourceName, const char* pathID /*= NULL*/, bool bWriteEmptySubkeys /*= false*/)
{
	// create a write file
...
{
	return SaveToFile(filesystem, resourceName, pathID, false);
}
...
void CUtlBuffer::Get(void* pOut, int size)
{
}
...
void V_MakeAbsolutePath(char* dst, int dstLen, const char* relative, const char* root) {}
void V_AppendSlash(char* path, int len) {}
void V_FixupPathName(char* dst, unsigned int dstLen, const char* src) {}
void CRC32_Init(unsigned long* crc) {}
void CRC32_ProcessBuffer(unsigned long* crc, const void* buf, int size) {}
void CRC32_Final(unsigned long* crc) {}
unsigned long (*g_BitWriteMasks)[33] = nullptr;

#include "tier3/mdlutils.h"
CMDL::~CMDL() {}
void CMDL::Draw(const matrix3x4_t& rootToWorld) {}
#include "materialsystem/MaterialSystemUtil.h"
void CTextureReference::Init(const char* pTextureName, const char* pTextureGroupName, bool bComplain) {}
...
static mstudioseqdesc_t g_dummySeqDesc;

mstudioseqdesc_t& CStudioHdr::pSeqdesc_Internal(int i)
{
	return g_dummySeqDesc;
}
