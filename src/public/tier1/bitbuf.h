...
inline void bf_write::WriteUBitLong( unsigned int curData, int numbits, bool bCheckRange )
{
	...
	extern unsigned long (*g_BitWriteMasks)[33];
...
{
	unsigned int ubitRet = ReadUBitLong( 6 );
	switch( ubitRet & ( 16 | 32 ) )
	{
		case 16:
			ubitRet = ( ubitRet & 15 ) | ( ReadUBitLong( 4 ) << 4 );
			Assert( ubitRet >= 16);
			break;
				
		case 32:
			ubitRet = ( ubitRet & 15 ) | ( ReadUBitLong( 8 ) << 4 );
			Assert( ubitRet >= 256);
			break;
		case 48:
			ubitRet = ( ubitRet & 15 ) | ( ReadUBitLong( 32 - 4 ) << 4 );
			Assert( ubitRet >= 4096 );
			break;
	}
	return ubitRet;
