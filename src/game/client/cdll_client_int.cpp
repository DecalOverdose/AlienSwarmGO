...
public:
	...
	virtual IReplayFactory* GetReplayFactory() { return NULL; };

	// Clear out the local player's replay pointer so it doesn't get deleted
	virtual void			ClearLocalPlayerReplayPtr() { /* Stubbed anyway*/ };

	virtual bool			ShouldDrawDropdownConsole() { return true; };
	...
	virtual void			WriteReplayScreenshot(WriteReplayScreenshotParams_t& params) { /*We don't even use RePlay*/ };
...
public:
	...
	virtual bool ValidateSignedEvidenceHeader(char const* szKey, void const* pvHeader, CDemoPlaybackParameters_t* pPlaybackParameters) { return true; };
	virtual void PrepareSignedEvidenceData(void* pvData, int numBytes, CDemoPlaybackParameters_t const* pPlaybackParameters) {};
	virtual bool ShouldSkipEvidencePlayback(CDemoPlaybackParameters_t const* pPlaybackParameters) { return true; };

	virtual void OnCommandDuringPlayback(char const* cmd) {};

	virtual void RetireAllPlayerDecals(bool bRenderContextValid) {};

	virtual void EngineGotvSyncPacket(const CEngineGotvSyncPacket* pPkt) OVERRIDE { /*Already stubbed, plus we don't use Protobuf*/ };

	virtual void OnTickPre(int tickcount) OVERRIDE {/*Already stubbed in the CS:GO codebase*/};
...
int CHLClient::Init(CreateInterfaceFn appSystemFactory, CGlobalVarsBase* pGlobals)
{
	...
	if ((blackboxrecorder = (IBlackBox*)appSystemFactory(BLACKBOX_INTERFACE_VERSION, NULL)) == NULL)
		return false;
	// DecalOverdose: Our mod isn't using XBOX Live
#if 0
...
int CHLClient::GetInEyeEntity() const
{
	...
	C_BaseEntity* pEntity = view->GetCurrentlyDrawingEntity();
	C_BasePlayer* player = ToBasePlayer(pEntity);
	if (player != nullptr)
	{
