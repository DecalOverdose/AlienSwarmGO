...
void CLocatorTarget::SetBinding( const char *pszBinding )	
{
	...
	while ( pchToken )
	{
		...
		const char *pchBinding = engine->Key_LookupBindingEx( szToken, nSlot, iTokenBindingCount );
		...
		while ( m_iBindingChoicesCount < MAX_LOCATOR_BINDINGS_SHOWN && pchBinding )
		{
			...
			pchBinding = engine->Key_LookupBindingEx( szToken, nSlot, iTokenBindingCount );
		}
